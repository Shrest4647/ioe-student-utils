...
export default function CollegesDashboardPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "colleges", debouncedSearch],
    queryFn: async () => {
  ...
  const colleges = data?.success ? data.data : [];
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <CardDescription>Manage existing college entries.</CardDescription>
          </div>
          <Button asChild>
            <Link href="/dashboard/colleges/new">
                ...
                {isLoading ? (
                  ...
                  colleges.map((college) => (
                    <TableRow key={college.id}>
                      ...
                      <TableCell>{college.university?.name || "N/A"}</TableCell>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" asChild>
                          <Link href={`/dashboard/colleges/${college.slug}`}>
