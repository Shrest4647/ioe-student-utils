...
export default function ProgramsDashboardPage() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["admin", "programs", debouncedSearch],
    queryFn: async () => {
  ...
  const programs = data?.success ? data.data : [];
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          <div>
            ...
            <CardDescription>
              Manage existing academic program entries.
            </CardDescription>
          </div>
          <Button asChild>
                    ...
                    <TableRow key={program.id}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="icon" asChild>
                          <Link href={`/dashboard/programs/${program.code}`}>
