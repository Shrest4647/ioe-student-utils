  ...
  for (const country of countries) {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/mcp/countries`,
        {
          method: "POST",
...
async function importDegrees() {
  ...
  for (const degree of degrees) {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/mcp/degrees`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
...
async function importFields() {
  ...
  for (const field of fields) {
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:3000"}/api/mcp/fields-of-study`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
